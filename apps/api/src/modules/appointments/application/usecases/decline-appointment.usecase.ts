...
@injectable()
export class DeclineAppointmentUseCase extends BaseUseCase<Input, Output> {
  ...
  protected async handle(input: Input, tx: TxClient): Promise<Output> {
    ...
    if (appointment.slotDoctorId !== input.doctorId) {
      throw new AppError('No tienes permiso para rechazar esta cita', 403)
    }

    await this.appointments.updateSlotStatus(appointment.eventId, 'available', tx)
    await this.appointments.deleteById(input.appointmentId, tx)
